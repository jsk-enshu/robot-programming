...
ロボティクス分野では現在`ROS 1`から`ROS 2`への移行期にあり多くの研究室や企業で両方のバージョンが共存している．本演習の1日目で扱ったメカトロボットでは`rosserial`（`ROS 1`）と`ros2_control`（`ROS 2`）を組み合わせたシステムを構築し`ros1_bridge`を使用してこれらを統合する．
