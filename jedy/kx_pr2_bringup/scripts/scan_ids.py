...
try:
    rospackage_path = rospack.get_path('kxr_models')
except rospkg.ResourceNotFound as e:
    print(Fore.RED)
    print("The ROS package 'kxr_models' could not be found."
          + " You need to make sure your ROS workspace is sourced (enabled) in the terminal.\n"
          + "Try running the following command:\n\n"
          + "source ~/ros/enshu_ws/devel/setup.bash\n\n"
          + "Alternatively, run\n\n"
          + "'roscd kxr_models' to check if the package is installed correctly.")
    print(Fore.RESET)
    sys.exit(1)
...
config_path = osp.join(
    rospackage_path,
    'config',
    # 'kxrmw4a6h2m_servo_config.yaml'
    'kxrl2l2a6h2m_servo_config.yaml'
)
